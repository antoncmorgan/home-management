<template>
    <div class="meal-plan-calendar-container">
        ...
        <n-table class="meal-plan-table" striped>
            <thead>
                <tr>
                    <th class="meal-row-header"></th>
                    <th v-for="day in days" :key="day">{{ day }}</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="meal in meals" :key="meal">
                    <td class="meal-row-header"><n-text strong>{{ meal.charAt(0).toUpperCase() + meal.slice(1)
                            }}</n-text></td>
                    <td v-for="day in days" :key="day">
                        <n-space vertical size="small" align="stretch" class="meal-events">
                            <template v-for="event in getMealEventsForDay(day, meal)"
                                :key="event.title + event.memberId">
                                <MealPlanEventItem :title="event.title" :color="event.color"
                                    :memberName="event.memberName" :itemsToPack="event.itemsToPack" />
                            </template>
                            <n-button v-if="getMealEventsForDay(day, meal).length === 0" class="add-meal-btn" tertiary
                                :block="true" @click="onAddMeal(day, meal)">
...
import MealPlanEventItem from './MealPlanEventItem.vue';
...
function getMealEventsForDay(day: DayType, meal: MealType) {
    ...
    return plans.map(plan => {
        ...
        // If lunch, get itemsToPack from mealObj (assume mealObj.itemsToPack exists)
        if (meal === 'lunch' && mealObj) {
            if (Array.isArray(mealObj.itemsToPack)) {
                itemsToPack = mealObj.itemsToPack;
            }
        }
        return {
